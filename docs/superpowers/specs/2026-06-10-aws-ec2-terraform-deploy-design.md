...
~/aws-infra/
├── global/
│   ├── tf-state/      # state용 S3 버킷 (bootstrap: local apply 후 자기 state를 S3로 migrate)
│   └── github-oidc/   # GitHub OIDC provider (계정당 1개 — 공용 스택으로 분리)
└── kkoma-slack/       # 이 프로젝트 스택
```

공용 `modules/`는 만들지 않는다 — 두 번째 프로젝트에서 반복 패턴이 보일 때 추출한다 (YAGNI).
...
2. `~/aws-infra` 저장소 생성, `global/tf-state` 스택을 local state로 apply → backend 블록 추가 후 `terraform init -migrate-state`로 자기 state를 S3로 이전 → `global/github-oidc` 스택 apply
