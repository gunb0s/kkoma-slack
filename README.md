...
- `/kkoma top`: 현재 채널의 가까운 추측 TOP 10
...
같은 서버/엔드포인트에서 영어 Semantle을 `/sema`로 제공합니다 (start/guess/top/status/hint/giveup, 메시지는 한글). 외부 `legacy.semantle.com` API(`/model2`, `/nearby_1k`)에 의존하며 정답 리스트는 `data/en/secrets.txt`에 번들됩니다.

- 한 채널에서는 `/kkoma`와 `/sema` 중 **하나만 동시 진행**할 수 있습니다. 진행 중인 게임을 풀거나 `giveup` 한 뒤 다른 게임을 시작하세요.
- 모든 게임은 `start`로만 시작됩니다. start 전에 추측하면 안내 메시지가 나옵니다.
- `KKOMA_ENABLE_SEMA=0`으로 끌 수 있습니다.
